...
def parse_args():
    ...
    # 数据路径参数
    parser.add_argument("--pt_data_path", type=str, default='/home/syh123/workspace/Data_Filter/data_filter/intermediate_data/qwen7/rtlcoder_step2_feature_merge_pre_woNetlist.pt', help='中间数据文件路径')
    parser.add_argument("--json_save_path", type=str, default='/home/syh123/workspace/Data_Filter/data/RTLCoder12k/qwen7/woNetlist_20.jsonl', help='保存结果路径')
    ...
    # 采样参数 - 修改为百分比
    parser.add_argument("--sample_ratio", type=float, default=0.21, help='每个簇中采样的样本比例 (0-1之间)')
...
def sample_by_ratio_and_mean_rate(cluster_labels, mean_rate_list, sample_ratio):
    """在每个簇中采样前sample_ratio比例的mean_rate最小的样本"""
    ...
    # 在每个簇中选择前sample_ratio比例的mean_rate最小的样本
    selected_samples = {}
    cluster_stats = {}  # 用于记录每个簇的统计信息
    ...
    for cluster_id in range(num_clusters):
        ...
        if cluster_samples:  # 只处理有样本的簇
            ...
            sample_count = max(1, math.ceil(cluster_size * sample_ratio))
            
            # 选择前sample_count个
            selected_samples[cluster_id] = [idx for _, idx in cluster_samples[:sample_count]]
            
            # 记录统计信息
            cluster_stats[cluster_id] = {
                'total_samples': cluster_size,
                'sampled_count': sample_count,
                'sampling_ratio': sample_count / cluster_size
            }
        else:
    ...
    return selected_samples, cluster_stats
...
def main():
    ...
    # 步骤4: 在每个簇中采样前sample_ratio比例的mean_rate最小的样本
    print(f"步骤4: 在每个簇中采样前{args.sample_ratio:.1%}比例的mean_rate最小的样本...")
    selected_samples, cluster_stats = sample_by_ratio_and_mean_rate(cluster_labels, valid_mean_rate_list, args.sample_ratio)
    ...
    for cluster_id in sorted(cluster_stats.keys()):
        ...
        total_original_samples += stats['total_samples']
        total_samples += stats['sampled_count']
        
        if stats['total_samples'] > 0:
            non_empty_clusters += 1
            print(f"簇 {cluster_id}: 共{stats['total_samples']}个样本, 采样{stats['sampled_count']}个, 采样比例{stats['sampling_ratio']:.1%}")
    
    print(f"\n总体采样统计:")
    print(f"有样本的簇数量: {non_empty_clusters}")
    print(f"原始有效样本总数: {total_original_samples}")
    print(f"最终采样样本数: {total_samples}")
    print(f"总体采样比例: {total_samples/total_original_samples:.1%}")
